...
export default class Stage5Scene extends Phaser.Scene {
    ...
    makeTrailerTexture() {
        if (this.textures.exists("trailer_top"))
            return;
    ...
    update(time, dtMs) {
        ...
        this.ph += phDot * dt;
        if (this.v < -4)
            this.reverseDistance += Math.abs(this.v) * dt;
        this.layout();
    ...
    checkParked(dt) {
        const m = 26;
        const inBay = this.axle.x > BAY.x + m &&
    ...
    win() {
        ...
        this.time.delayedCall(1800, () => this.scene.start("StageCompleteScene", { stage: 7, score: 100, nextScene: "Stage6Scene" }));
    }
}
