...
export default class Stage2Scene extends Phaser.Scene {
    ...
    drawStopLine(x, y, vertical) {
        ...
        if (vertical)
            g.fillRect(x - 100, y, 200, 10);
        else
            g.fillRect(x, y - 100, 10, 200);
    }
    drawStopSign(x, y) {
    ...
    update(time, dtMs) {
        ...
        if (!onRoad) {
            ...
            if (this.offroadTimer > 2.2 || this.distanceToRoad(this.vehicle.x, this.vehicle.y) > 260)
                this.failOffroad();
        }
        else {
            this.offroadTimer = 0;
    ...
    checkArrival() {
        ...
        if (d < this.dest.r) {
            ...
            this.time.delayedCall(1400, () => this.scene.start("StageCompleteScene", { stage: 4, score, nextScene: "Stage3Scene" }));
        }
    }
    failOffroad() {
        this.finished = true;
