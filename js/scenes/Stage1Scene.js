...
export default class Stage1Scene extends Phaser.Scene {
    ...
    drawRing(hs) {
        ...
        if (hs.seen)
            color = COLORS.good;
        if (this.phase === "startup" && this.targetId === hs.control.id)
            color = COLORS.accent;
    ...
    finishStartup() {
        ...
        this.time.delayedCall(1800, () => this.scene.start("StageCompleteScene", { stage: 1, score: 100, nextScene: "StageGearScene" }));
    }
}
