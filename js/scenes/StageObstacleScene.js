...
export default class StageObstacleScene extends Phaser.Scene {
    ...
    drawQuestion() {
        ...
        if (q.type === "grade")
            this.drawGrade(q.value);
        else if (q.type === "side")
            this.drawSideSlope(q.value);
        else if (q.type === "step")
            this.drawStep(q.value);
        else if (q.type === "trench")
            this.drawTrench(q.value);
        else
            this.drawLog(q.value);
    ...
    answer(passable) {
        ...
        if (this.index >= QUESTIONS) {
            ...
            this.time.delayedCall(900, () => this.scene.start("StageCompleteScene", { stage: 3, score, nextScene: "Stage2Scene" }));
        }
        else {
