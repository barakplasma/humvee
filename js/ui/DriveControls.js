...
export default class DriveControls {
    ...
    releasePedal(kind, pointerId = null) {
        ...
        if (activeId !== null && pointerId !== null && activeId !== pointerId)
            return;
        if (cont)
            cont.setScale(1);
        if (kind === "gas") {
    ...
    bindPointerControls() {
        ...
        this._pointerDownHandler = (p) => {
            ...
            if (this.selectFromPointer(p, inRect))
                return;
            if (this.gasPedal && inRect(p, this.gasPedal._hit))
                this.pressPedal("gas", p.id);
            else if (this.brakePedal && inRect(p, this.brakePedal._hit))
                this.pressPedal("brake", p.id);
            else if (inCircle(p, this.wheelX, this.wheelY, this.wheelR + 28))
                this.startWheelDrag(p);
        };
        this._pointerMoveHandler = (p) => {
            if (this._dragPointerId === p.id)
                this.updateSteerFromPointer(p);
        };
    ...
    buildSelectors() {
        this.transferSelector = this.makeSelector(40, 120, t("transfer_short"), TRANSFER, this.range, (v) => {
            this.range = v;
            if (this.opts.onRange)
                this.opts.onRange(v);
        });
        this.transSelector = this.makeSelector(170, 120, t("transmission_short"), TRANSMISSION, this.gear, (v) => {
            this.gear = v;
            if (this.opts.onGear)
                this.opts.onGear(v);
        });
    ...
    selectFromPointer(p, inRect) {
        ...
        for (const selector of selectors) {
            if (!selector || !selector._hitCells)
                continue;
    ...
    setGear(v) {
        this.gear = v;
        if (this.transSelector)
            this.setSelector(this.transSelector._cells, v);
    }
    setRange(v) {
        this.range = v;
        if (this.transferSelector)
            this.setSelector(this.transferSelector._cells, v);
    }
    ...
    setSpeedDisplay(kph) {
        ...
        if (rpm !== null)
            parts.push(`${t("hud_rpm")} ${Math.round(rpm)}`);
    ...
    getRpm(speed, { load = 0 } = {}) {
        const gear = GEAR_SPEC[this.gear] || GEAR_SPEC.N;
        const range = RANGE_SPEC[this.range] || RANGE_SPEC.H;
        if (gear.drive === 0 || range.speed === 0)
            return gear.idle + this.throttle * (gear.redline - gear.idle) * 0.35;
    ...
    bindTilt() {
        ...
        const handler = (e) => {
            if (e.beta === null && e.gamma === null)
                return;
            const angle = (typeof screen !== "undefined" && screen.orientation && screen.orientation.angle) ||
                window.orientation ||
                0;
            let tilt;
            if (angle === 90)
                ...
                tilt = e.gamma || 0; // portrait / default
            this._rawTilt = tilt;
            if (this._tiltNeutral === null)
                this._tiltNeutral = tilt;
            ...
            if (Math.abs(delta) < DEADZONE_DEG)
                ...
                delta -= Math.sign(delta) * DEADZONE_DEG;
    ...
    recenterTilt() {
        if (this._tiltActive)
            this._tiltNeutral = this._rawTilt;
    }
    ...
    bindGamepad() {
        ...
        if (this._gamepad && this._gamepad.setAxisThreshold)
            this._gamepad.setAxisThreshold(0.12);
        this._gamepadConnectedHandler = (pad) => {
            this._gamepad = pad;
            if (pad && pad.setAxisThreshold)
                pad.setAxisThreshold(0.12);
        };
        this._gamepadDisconnectedHandler = (pad) => {
            if (this._gamepad === pad)
                this._gamepad = this.getActiveGamepad(pad);
        };
    ...
    getActiveGamepad(excludePad = null) {
        ...
        if (!pads.length)
            pads = [plugin.pad1, plugin.pad2, plugin.pad3, plugin.pad4].filter(Boolean);
        if (!pads.length && plugin.gamepads)
            pads = plugin.gamepads;
    ...
    getGamepadState() {
        ...
        this._gamepad = pad;
        if (!pad)
            return { steer: 0, gas: false, brake: false };
        const leftX = (pad.leftStick && Number.isFinite(pad.leftStick.x) && pad.leftStick.x) ||
    ...
    getGamepadAxisValue(pad, index) {
        if (!pad.getAxisValue)
            return 0;
        if (pad.getAxisTotal && index >= pad.getAxisTotal())
            return 0;
        return pad.getAxisValue(index) || 0;
    }
    getGamepadButtonValue(pad, index) {
        if (pad.getButtonTotal && index >= pad.getButtonTotal())
            return 0;
        if (pad.getButtonValue)
            return pad.getButtonValue(index) || 0;
    ...
    update(dtMs) {
        ...
        if (kbSteer !== 0)
            target = kbSteer;
        else if (this._dragPointerId !== null)
            target = this._dragSteer;
        else if (gp.steer !== 0)
            target = gp.steer;
        else if (this._tiltActive)
            target = this._tiltSteer;
        else
            target = 0;
    ...
    destroy() {
        ...
        if (this._tiltPermissionHandler)
            this.scene.input.off("pointerdown", this._tiltPermissionHandler);
        ...
        if (this._tiltHandler)
            window.removeEventListener("deviceorientation", this._tiltHandler, true);
